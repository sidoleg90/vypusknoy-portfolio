...
сделанными персональным AI-агентом за 7,5 недель. Показывается **локально** на защите
(файл открывается в браузере). Публиковать в интернет НЕ нужно — внутри реальные внутренние
цифры Линдэйли.
...
## Приватность — ВАЖНО
Показ **локальный**. В контенте есть реальные внутренние данные Линдэйли (портфель 113,8 млн ₽,
имена клиентов, договорный рычаг 1 014 897 ₽). **Не публикуй сайт в открытый интернет** без
обезличивания. Приватные GitHub-репозитории не линкуем как код — только публичный обзор
`lindaily-guide`.

## Что можно улучшить (необязательно)
- Превью для флагмана «Парсер ЛПР» (сейчас плейсхолдер — бот приватный, скриншот чата снять руками).
